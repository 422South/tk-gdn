...
class GDNEngine(sgtk.platform.Engine):
    """
    An GDN engine for Shotgun Toolkit.
    """

    # the maximum size for a generated thumbnail
    MAX_THUMB_SIZE = 512

    SHOTGUN_GDN_HEARTBEAT_INTERVAL = 1.0
    SHOTGUN_GDN_HEARTBEAT_TOLERANCE = 2
    SHOTGUN_GDN_NETWORK_DEBUG = "SHOTGUN_GDN_NETWORK_DEBUG" in os.environ
    ...
    _SHOTGUN_GDN_PORT = os.environ.get("SHOTGUN_GDN_PORT")
    _SHOTGUN_GDN_APPID = os.environ.get("SHOTGUN_GDN_APPID")
    ...
    def post_context_change(self, old_context, new_context):
        ...
        #     self.gdn.dollar.setenv("TANK_CONTEXT", new_context.serialize())
    ...
    def pre_app_init(self):
        ...
        self.__tk_gdn = self.import_module("tk_gdn")
        ...
        # get the GDN instance. it may have been initialized already by a
        # previous instance of the engine. if not, initialize a new one.
        self._gdn = self.__tk_gdn.GDNBridge.get_or_create(
            identifier=self.instance_name,
            port=self._SHOTGUN_GDN_PORT,
            logger=self.logger,
            network_debug=self.SHOTGUN_GDN_NETWORK_DEBUG,
        )

        self.logger.debug("Network debug logging is %s" % self._gdn.network_debug)

        self.logger.debug("%s: Initializing..." % (self,))

        # connect to all the gdn bridge signals
        self.gdn.logging_received.connect(self._handle_logging)
        self.gdn.command_received.connect(self._handle_command)
        self.gdn.active_document_changed.connect(self._handle_active_document_change)
        self.gdn.run_tests_request_received.connect(self._run_tests)
        self.gdn.state_requested.connect(self.__send_state)
        self.gdn.hwnd_changed.connect(self.__handle_hwnd)

        # in order to use frameworks, they have to be imported via
        # import_module. so they're exposed in the bundled python.
        shotgun_data = self.__tk_gdn.shotgun_data
        settings = self.__tk_gdn.shotgun_settings
        # keep a handle for shotgun globals as they are needed in other
        # functions as well
        self.__shotgun_globals = self.__tk_gdn.shotgun_globals
    ...
    def post_app_init(self):
        ...
        """
        if not self.gdn.event_processor:
            try:
                from sgtk.platform.qt import QtGui

                self.gdn.event_processor = QtGui.QApplication.processEvents
            except ImportError:
                pass
        ...
        self.gdn.send_log_file_path(log_file)
        ...
        self.logger.debug("Init: clearing stored context cache.")
    ...
    def destroy_engine(self):
        ...
        # instead of GDN's application window.
        if self._PROXY_WIN_HWND and sgtk.util.is_windows():
            self.__tk_gdn.win_32_api.SetParent(self._PROXY_WIN_HWND, 0)
        ...
        self.gdn.logging_received.disconnect(self._handle_logging)
        self.gdn.command_received.disconnect(self._handle_command)
        self.gdn.active_document_changed.disconnect(
            self._handle_active_document_change
        )
        self.gdn.run_tests_request_received.disconnect(self._run_tests)
        self.gdn.state_requested.disconnect(self.__send_state)
    ...
    def register_command(self, name, callback, properties=None):
        """
        Registers a new command with the engine. For GDN RPC purposes,
        a "uid" property is added to the command's properties.
        """
        properties = properties or dict()
        properties["uid"] = self.__get_command_uid()
        return super(GDNEngine, self).register_command(
    ...
    def host_info(self):
        ...
        :returns: A {"name": application name, "version": application version}
                  dictionary. eg: {"name": "GDN", "version": "6.0.302"}
        """
        if not self.gdn:
            # Don't error out if the bridge was not yet started
            return {"name": "422 Global Data Navigator", "version": "unknown"}
        theApp = self.gdn
    ...
    def _check_connection(self):
        """Make sure we are still connected to the gdn product."""
        ...
        try:
            ...
            if self._FAILED_PINGS >= self.SHOTGUN_GDN_HEARTBEAT_TOLERANCE:
                from sgtk.platform.qt import QtCore
    ...
    def __handle_hwnd(self, hwnd):
        """

        :param hwnd: The GDN Main Frame windows hwnd
        :return: nothing
        """
    ...
    def _run_tests(self):
        ...
        # try:
        #     tests_root = os.environ["SHOTGUN_GDN_TESTS_ROOT"]
        # except KeyError:
        #     self.logger.error(
        #         "The SHOTGUN_GDN_TESTS_ROOT environment variable "
        ...
        #             "specified by the SHOTGUN_GDN_TESTS_ROOT "
    ...
    def app_id(self):
        ...
        TODO
        """
        return self._SHOTGUN_GDN_APPID
    ...
    def _define_qt_base(self):
        ...
        * dialog_base - base class for to use for Toolkit's dialog factory
        ...
        base = super(GDNEngine, self)._define_qt_base()
        if not base:
    ...
    def __check_for_popups(self):
        ...
        #     # dialog classes.
        #     hwnds = self.__tk_gdn.win_32_api.find_windows(
        ...
        #     all_hwnds = {}
        #     GetWindow = self.__tk_gdn.win_32_api.ctypes.windll.user32.GetWindow
        #     for hwnd in hwnds:
        ...
        #         self.__tk_gdn.win_32_api.bring_to_front(all_hwnds[hwnd_long])
    ...
    def _win32_get_gdn_main_hwnd(self):
        """
        Windows specific method to find the GDN main window
        handle (HWND)
        """

        return self._WIN32_GDN_MAIN_HWND

    def _win32_get_proxy_window(self):
        """
        Windows-specific method to get the proxy window that will 'own' all
        Toolkit dialogs.  This will be parented to the main
        application.

        :returns: A QWidget that has been parented to After Effects's window.
        """
        # Get the main After Effects window:
        ps_hwnd = self._win32_get_gdn_main_hwnd()
        ...
        if ps_hwnd:
            ...
            if QtCore.__version__.startswith("4."):
                proxy_win_hwnd = self.__tk_gdn.win_32_api.qwidget_winid_to_hwnd(
        ...
        if proxy_win_hwnd is None:
            ...
            win_ex_style = self.__tk_gdn.win_32_api.GetWindowLong(
                proxy_win_hwnd, self.__tk_gdn.win_32_api.GWL_EXSTYLE,
            )

            self.__tk_gdn.win_32_api.SetWindowLong(
                proxy_win_hwnd,
                self.__tk_gdn.win_32_api.GWL_EXSTYLE,
                win_ex_style | self.__tk_gdn.win_32_api.WS_EX_NOPARENTNOTIFY,
            )
            self.__tk_gdn.win_32_api.SetParent(proxy_win_hwnd, ps_hwnd)
    ...
    def __send_state(self):
        ...
        self.gdn.send_commands(all_commands)
    ...
    def __setup_connection_timer(self, force=False):
        ...
        if self._CHECK_CONNECTION_TIMER is None or force:
            ...
            timer.start(self.SHOTGUN_GDN_HEARTBEAT_INTERVAL * 1000.0, )
    ...
    def __request_context_display(self, entity):
        ...
        if not entity:
            ...
            self.gdn.send_context_display(fields_html)
