...
class AfterEffectsStartVersionControlPlugin(HookBaseClass):
    ...
    def accept(self, settings, item):
        ...
        if path:
            ...
            if version_number is not None:
                self.logger.info(
                    "GDN '%s' plugin rejected project" % self.name
                )
            ...
            self.logger.warn(
                "GDN project has not been saved.",
                extra=self.__get_save_as_action(),
            )

        self.logger.info(
            "GDN '%s' plugin accepted the project." % self.name,
    ...
    def validate(self, settings, item):
        ...
        if not path:
            ...
            # validation fails
            error_msg = "The GDN project has not been saved."
    ...
    def publish(self, settings, item):
        ...
        self.logger.info(
            "A version number has been added to the GDN project..."
        )
        self.logger.info("  GDN project path: %s" % (version_path,))
